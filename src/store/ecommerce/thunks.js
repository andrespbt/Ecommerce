...
import { loadLikedProducts } from '../../helpers/loadProducts';
import { addNewLikedProduct, deleteLikedProductById, isSaving, loadProducts } from './ecommerceSlice';

export const startLoadingProducts = () => {
  return async (dispatch, getState) => {
    const { uid } = getState().auth;
    const likedProducts = await loadLikedProducts(uid);
    dispatch(loadProducts({ likedProducts }));
  };
};
